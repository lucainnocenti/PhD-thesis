...
\end{gathered}
\end{equation}
we can rewrite the conditions as the following system of quadratic equations:
\begin{equation}
    \tilde{\bs d}^\dagger
    \begin{pmatrix}
        Q_s & \bs\beta_s \\
...
For example, in the special case of $u_1, u_2, u_3 \in \RR$, $u_3 \ge 0$, this probability is
\begin{equation}
\begin{aligned}
...
Apart from the constraints we have to impose on the target $\bs u$ (implying that not all targets are reachable when these degenerate conditions are met), it is interesting to note that while there is here in principle an infinite number of possible solutions, as $d_I$ can have any real value, the physical constraint of the corresponding set of amplitudes making up a normalised state forces the value of $d_I$ to satisfy
$d_I^2 = 1/2 - u_{1I}^2 - u_{2R}^2/4$.
