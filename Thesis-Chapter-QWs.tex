...
\begin{equation}
\begin{aligned}
	\ket\Phi &= N \left( u_1 \ket{1,\uparrow} + u_{n+1} \ket{n+1,\downarrow}
	+ \sum_{i=2}^n \left[
...
\end{equation}
where $(d_i)_{i=2}^n$ are complex parameters to be determined, and $N\in\RR$ is the normalisation constant.
...
Isolating the unknown from the rest of the parameters, we rewrite~\cref{eq:qw_conditions_for_ds} as
\begin{equation}
    2\sum_{i=2}^s d_i^* d_{n-s+i} + \sum_{i=1}^s u_i^* u_{n-s+i}
    - \sum_{i=n-s+1}^{n} u_{i-n+s}^* d_i
    - \sum_{i=2}^{s+1} u_{n-s+i} d_i^* = 0.
\end{equation}
Then, defining
\begin{equation}
\begin{gathered}
    Q_s = 2\sum_{i=2}^s \ketbra{i}{n-s+i}, \\
    \bs\alpha_s = -\sum_{i=n-s+1}^n u_{i-(n-s)}\ket{i},
    \qquad
    \bs\beta_s = -\sum_{i=2}^{s+1} u_{i+(n-s)}\ket{i}, \\
    C_s = \sum_{i=1}^s u_i^* u_{n-s+i},
    \qquad
    \tilde{\bs d} = (\bs d, 1)^T,
\end{gathered}
\end{equation}
...
\end{equation}
Note that this does not imply that for $\lvert u_1 \rvert = \lvert u_3 \rvert$ there is no solution for $d_2$, but only that \cref{eq:qw_solution_for_d2} does not apply in that case.
...
As already mentioned in~\cref{sec:qw_focusing_walker_states}, for 2 steps the reachability conditions read
\begin{equation}
	u_1^* (u_2 - d_2) + d_2^* u_3 = 0,
	\label{eq:qw_reachability_condition_2steps}
\end{equation}
...
\begin{equation}
	...
	\label{app:eq:qw_solution_for_d2}
\end{equation}
...
On the other hand, for $u_1 = -u_3$, the probability does not vanish, which may be puzzling because in this case \cref{app:eq:qw_solution_for_d2} itself is singular.
A more careful analysis of \cref{app:eq:qw_solution_for_d2} shows however that $u_1 = -u_3$ corresponds to a removable singularity, consistently with the corresponding non-vanishing projection probability.

When the $\lvert u_1 \rvert \neq \lvert u_3 \rvert$ condition is not met, the solution space changes significantly.
We here consider for example the case $u_1 = u_3 \neq 0$. Substituting this into \cref{eq:qw_reachability_condition_2steps} we get the conditions
\begin{equation*}
	\begin{dcases}
...
\begin{figure}[tb]
	...
	\caption{
		...
		Starting from \cref{eq:state_degenerate_case}, we use as target state the normalised vector
