...
In this section we present a full characterisation of the output states of a finite number of steps of a time-dependent \ac{DTQW}. We will find that such a characterisation can be formulated in terms of a sequence of efficiently verifiable conditions on the amplitudes of the output state.
More precisely, given a candidate output state, these conditions can be used to 1) verify whether there is a sequence of coin operations generating the target states after the prescribed number of steps, and 2) if the answer is positive, obtain a sequence of coin operations generating it.

\subsection{1-step reachability condition}
\label{sec:1step_reachability}

To derive the result, we start by analysing what happens for small numbers of steps, to better illustrate the important features of the quantum walk.

Suppose the walker is initially localised at the site $i=1$ with some unspecified initial coin state $\ket s$.
% $\ket s=u_{1,\uparrow}^{(0)}\ket{1,\uparrow} + u_{1,\downarrow}^{(0)}\ket{1,\downarrow}$.
The initial state of the full system is then
\begin{equation}
    \ket*{\Psi^{(0)}} \equiv
    \ket1\otimes\ket s \equiv
    \ket{1}\otimes(u^{(0)}_{1,\uparrow} \ket{\uparrow} +
    u^{(0)}_{1,\downarrow} \ket{\downarrow}).
\end{equation}
Let $\calC_1$ denote the coin operation performed at the first step,
and let $\calW_{\calC_1}$ be the corresponding step operation.
Then, the state after the first step is
$\ket*{\Psi^{(1)}} \equiv \calW_{\calC_1} \ket*{\Psi^{(0)}}$.
Explicitly, this state reads
\begin{equation}
    \ket*{\Psi^{(1)}} =
...
\end{equation}
Note how this feature is independent on the initial coin state $\ket s$ or the coin operation $\calC$, but is rather a general property of states produced by this kind of \ac{QW} dynamic.
...
\end{equation}
% $
% \mel{1, \downarrow}{\mathcal W}{\Psi} =
% \mel{n+1, \uparrow}{\mathcal W}{\Psi} = 0
% $.
