...
However, when specific conditions are met, a full reconstruction is still possible in these circumstances~\cite{banchi2018multiphoton}.
...
The output probabilities corresponding to a measurement in the computational basis following the application of a unitary $\calU$ can be written as
\begin{equation}
...
We can represent $\Psi$ as a (generally non-square) matrix, for example by describing it via its action on the coefficients of $\rho$ in some operatorial basis.
...
Indeed, applying \ac{PCA} to this experimental dataset, three principal components are found to sufficient to capture most of the information. Projecting the images along the corresponding axes, we find that, in the reduced representation, the dataset is arranged in good approximation on a three-dimensional sphere.
Remarkably, this was not obvious from the experimental dataset alone, but was easily found via \ac{DR}. This result highlights the potential of \ac{DR} to reveal features of the states generating a given experimental dataset, also in the presence of experimental noisy conditions.
To assess the accuracy of such reconstruction, we computed the average fidelity $\calF_{\on{avg}}$ between the expected state and the one found by our analysis with PCA.
The fidelity is found to be $\calF_{\on{avg}}\sim0.96$, with standard deviation $\sim0.01$, thus certifying the quality of the reconstruction.
The full histogram of fidelities is shown in~\cref{fig:VVBs:PCAresults}\textbf{c}, 
...
To illustrate the usefulness of these ideas in understanding the type of states generated by a given apparatus, we consider here two examples of applications of \ac{PCA} for different types of input states. 
...
Applying PCA to this dataset, we find six non-vanishing singular values, whose associated principal components are given in~\cref{fig:S1}\textbf{a}.
...
On the other hand, if the dataset under consideration consists of states in the space $\mathcal S_2=\{c_1\ket{L,1}+c_2\ket{R,2}, c_3\ket{L,3}+c_4\ket{R,4}\}$, then \ac{PCA} finds \emph{seven} principal components associated with non-vanishing singular values, as again shown in~\cref{fig:S1}\textbf{a}.
...
This method thus provides a quick and easy way to obtain useful information about the dimensionality of generated states, as well as about other properties such as specific symmetries, which, as shown in~\cref{fig:S1}, are often picked up by the principal components.
...
\acfp{SVM} are a class of \emph{supervised learning} algorithms, whose goal is to classify data into discrete classes. \acp{SVM} work by finding an partition of the feature space such that all the vectors with the same training label lie on the same classification sector, if at all possible.
In particular, \emph{linear} \acp{SVM} aims to separate the data linearly, that is, look for an optimal separating hyperplane.

\tmpHeading{The math of linear SVMs}
SVMs take as input a training set of labelled data of the form $\{(\bs x_1,\ell_1),...,(\bs x_n,\ell_n)\}$, where $\bs x_i\in\RR^N$ and $\ell_i$ is one of two possible labels. If there are only two possible labels, we conventionally assume $\ell_i\in\{-1,1\}$.
