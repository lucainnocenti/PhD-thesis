...
The free parameters of the CNN, that is, all the weights and biases connecting the neurons at different layers, as well as the values of the filters, are trained using \ac{SGD}. This is the same algorithm described in~\cref{subsec:GL:supervised_learning_and_SGD}.
...
\end{equation}
where the real parameter $\eta>0$ is the \emph{learning rate}, used to determine the speed of the training.
...
An updating rule that is often used in state-of-the-art application is referred to as the \textsc{Adam}~\cite{ruder2016overview}, which uses different learning rates for different parameters, and adapts it conditionally to the recent training history of that parameter.
\highlight{(reread)}
...
We first showcase CNNs to classify experimental and simulated VVBs into $15$ different classes characterised by their OAM numbers $(m_1, m_2)$, as per~\cref{fig:VVBs:simVsExp}.
For each class we generate states with $\theta=\pi/2$ and $\phi\in[0,2\pi]$.
% The size of the training set is $400$ images per class.
We find that training and testing CNNs with simulated images easily leads to $100\%$ accuracy. This remains true when the training and testing datasets comprise simulated images with added white noise.
...
On the other hand, we find that training on simulated images is not enough to teach the network to categorise experimental ones. A possible reason for this is that experimental images can be quite different than the simulated ones, as can be already seen in the images provided in~\cref{fig:VVBs:simVsExp}. Indeed, the simulated images assume a perfect superposition of states with two different OAM numbers, whereas the states produced by \acp{QP} are closer to \emph{Hypergeometric-Gaussian modes}~\cite{karimi2007hypergeometricgaussian}. These are a different family of solutions of the Helmoltz equation in cylindrical coordinates. We show a few examples of such beams in~\cref{fig:VVBs:hyGG}.
In this work we stick with the simpler OAM beams for simplicity.

\begin{figure}[tb]
    \centering
    \includegraphics[width=0.8\linewidth]{Figures/VVBs/VVBs-hyGG.pdf}
    \caption{
    	asd
    }%
    \label{fig:VVBs:hyGG}
