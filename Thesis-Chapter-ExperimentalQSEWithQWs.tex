...
\tmpHeading{What is this chapter about} In this section we present an experimental implementation of the state-engineering scheme explained in~\cref{chapter:quantum_walks}, leveraging the \emph{coin space} of a quantum walk as an auxiliary degree of freedom to engineer arbitrary qudit states.
In particular, we use an experimental apparatus implementing time-dependent \acp{QW} in the polarisation and \ac{OAM} degrees of freedom~\cite{zhang2010implementation,goyal2013implementing,cardano2015quantum}, as made possible by the use of \acp{QP}~\cite{marrucci2006optical}, to engineer qudit state in the \ac{OAM} degree of freedom of single photons.
The work described in this chapter is also found in~\cite{giordani2019experimental}.

\tmpHeading{Why \ac{OAM}-polarisation architecture} \ac{OAM}-based experiments offer the possibility to explore Hilbert spaces of large dimensions in \emph{light} of the favourable scaling of the number of optical elements with the size of the walk~\cite{reck1994experimental,clements2016optimal}. Moreover, the scheme allows for the full control of the coin operation that is key to the walk implementation of the walk.
\highlight{should comment more about coolness of OAM}
...
\tmpHeading{Remember results of~\cref{chapter:quantum_walks}}
...
\caption{
	...
	\textbf{(a)} Representation on a Bloch-like ball of the four target states corresponding to the superposition of $\ket{\pm 5}$, which correspond corresponding to \ac{OAM} states with maximum and minimum projection of the angular momentum along the quantization axis.
...
To showcase the experimental engineering of arbitrary six-dimensional qudits, we focus on a few physically relevant classes of states.
First, we consider the synthesis of \emph{angular-momentum Schrödinger cat states}~\cite{militello2006distilling}, which are given by superpositions of extremal walker positions. The correspondence between the position space of the walker and an angular momentum of quantum number $n/2$, which will be explained later in this section, makes the \ac{QW} perfectly suited to synthesise this class of states. Schrödinger cat states play a crucial role in the investigations on foundations of quantum mechanics~\cite{schrodinger1935gegenwartige}\highlight{sbadabem} and their generation is at the core of various quantum engineering protocols~\cite{brune1992manipulation, monroe1996schrodinger,agarwal1997atomic, zhang2016creating}. The second class of states that we consider are \emph{spin-coherent states}~\cite{ulyanov1999spin}, which are the spin-like counterpart of \emph{coherent states} of a quantum harmonic oscillator. Finally, in order to validate the flexibility of our approach, we demonstrate high-quality engineering of both balanced, and randomly sampled states.

\section{Experimental apparatus}
\label{sec:expQWs:experimental_apparatus}

\tmpHeading{Experimental implementation of QW}
We implement a discrete-time \ac{QW} with $n=5$ steps, using the \ac{OAM} as the physical embodiment of the \emph{walker}, and the polarisation as the \emph{coin}.  are encoded in circular-polarisation states $\{|R\rangle, |L\rangle\}$. We dub such degree of freedom as \ac{SAM} to mark the difference with \ac{OAM}. Our experimental setup, which is shown schematically in Fig.~\ref{fig:expQWs:schematics} and follows Refs.~\cite{cardano2015quantum,cardano2016statistical}, allows for the full coin-walk evolution to take place in a single light beam, thus avoiding a nonlinear growth of optical paths as in previous interferometric implementations~\cite{zhang2010implementation,goyal2013implementing,cardano2015quantum}. Such scheme guarantees a linear scaling of the number of optical elements needed to implement a $n$-step \ac{QW} (see Ref.~\cite{SI}). Arbitrary coin operators are achieved through a sequence of suitably arranged and oriented \ac{QWP} and \ac{HWP}~\cite{simon1990minimal}. The shift operator $\calS$ is instead implemented using a \ac{QP}~\cite{marrucci2006optical}, an active device that uses an inhomogeneous birefringent medium to convert SAM into \ac{OAM} and that can conditionally change the values of the \ac{OAM} by a quantity $2q$ (here $q$ is the topological charge of the device) according to transformations 
\begin{equation}
\begin{aligned}
	|L,m\rangle & \stackrel{\text{QP}}{\longrightarrow} \cos{\frac{\delta}{2}}|L,m\rangle
	+ i e^{2 i \alpha_0}\sin{\frac{\delta}{2}}|R,m+2q\rangle, \\
	|R,m\rangle & \stackrel{\text{QP}}{\longrightarrow} \cos{\frac{\delta}{2}}|R,m\rangle
	+ i e^{-2 i \alpha_0}\sin{\frac{\delta}{2}}|L,m-2q\rangle.
\end{aligned}
\end{equation}
...
\tmpHeading{Photons generation scheme}
Single-photon states are generated via a type-II, collinear \ac{SPDC} source (see~\cref{fig:expQWs:schematics}).
The photons emitted by the source are separated with a polarising beam splitter (\ac{PBS}) and coupled to two \acp{SMF}. One photon acts as the trigger signal, while the other one undergoes the \ac{QW} evolution. After the propagation in the \ac{SMF} and the first \ac{PBS}, the initial state of the walker and coin is prepared in $\ket{\psi_0}_{wc}=\ket{0}_w \otimes \ket{+}_c$ with $\ket{+}_c=(\ket{{\uparrow}}_c+\ket{{\downarrow}}_c)/\sqrt2$. At the end of an $n$-step \ac{QW}, the protocol involves a projection of the coin state onto $|+\rangle_c$. This is experimentally implemented by a final \ac{PBS}. The \ac{OAM} analysis is performed through a \ac{SLM} followed by coupling into a single-mode fiber, which allows for the measurement of arbitrary superposition of \ac{OAM} components with high accuracy~\cite{bolduc2013exact,dambrosio2013test}. The quantum state fidelity between the actual state of the walker and the target $(n+1)$-dimensional state is estimated by projecting the \ac{OAM} state onto a basis that contains the given target state (see~\cref{fig:expQWs:schematics}).


\tmpHeading{Testability with SAM-\ac{OAM} (from other paper)}
The state engineering protocol here proposed can be tested with a DTQW in \ac{OAM} and \ac{SAM} of a photon, encoding position and coin degrees of freedom of the walker into \ac{OAM} and \ac{SAM} of light.
A feature of this scheme is that the evolution of the system lies in a single optical path~\cite{zhang2010implementation}, allowing for an implementation using a single photon.
A complete description of such a scheme is described in~\cref{chapter:experimental_engineering_qudits}.
...
With this encoding, arbitrary coin operations can be implemented using \acp{QWP} and \acp{HWP}.
The controlled shift operations are instead realized with \ac{QP}: a birefringent liquid-crystal medium that rises and lowers the value of $m$ conditionally to the polarization state~\cite{marrucci2006optical},
...
while the \ac{OAM} state can be analysed with a spatial light modulator (SLM)~\cite{cardano2015quantum}.
...
\caption{\highlight{to fix}
    ...
    The shift operation in \ac{OAM} space is implemented with a q-plate (QP), which shifts the \ac{OAM} of $\pm 2q$ conditionally to the polarization state of the photon.
...
In~\cref{fig:VVBs:figSpin}\textbf{f} quantum state fidelities are reported for all experimental engineered states, including the Fourier basis and randomly sampled qudits, where the red area shows the average fidelity and its uncertainty ($\mathcal{F}{=}0.954 \pm 0.001$)~\cite{SI}. Such test provides a further proof of the effectiveness of the strategy demonstrated in our experiment.
...
\tmpHeading{What are they used for?}

The second class of relevant states that we addressed are \acp{SCS}~\cite{agarwal1997atomic}. These are the counterparts of coherent states of the harmonic oscillator for a particle with spin $s$~\cite{radcliffe1971some,arecchi1972atomic,agarwal1997atomic,markham2003classicality}. \acp{SCS} are eigenstates -- with eigenvalue $s$ -- of the component of the total spin-momentum operator $\hat{S}$ pointing along the direction identified by the polar spherical angles $\{ \theta, \phi \}$~\cite{arecchi1972atomic,agarwal1997atomic,ulyanov1999spin,lee2015visualizing} 
...
\section{Scalability of the platform}
