...
More explicitly, LG modes are characterised, in cylindrical coordinates in the transverse propagation plane, by the amplitude profile $u_{p,\ell}(r,\theta)$ given by~\cite{doster2016laguerre–gauss}:
\begin{equation}
    u_{p,\ell}(r,\theta) = 
    % C_{p,\ell}
    \sqrt{\frac{2p!}{\pi (p+m)!}}
    \left(\frac{r\sqrt2}{w_0}\right)^{\lvert \ell\rvert} L_p^{\lvert \ell\rvert}(2r^2) e^{i\ell\theta},
\end{equation}
where $L_p^\ell$ are the generalised Laguerre polynomials of $p$ and $\ell$, and $w_0$ is the beam waist.
...
In~\cref{fig:expQWs:avgProbabilitiesVsStepNumber} we give the \textit{average} projection probability as a function of the number of steps.
...
\acfp{SCS} are the counterparts of coherent states of the harmonic oscillator for a particle with spin $s$~\cite{radcliffe1971some,arecchi1972atomic,agarwal1997atomic,markham2003classicality,chryssomalakos2018geometry}. They are the eigenstates, with eigenvalue $s$, of the component of the total spin-momentum operator along the direction identified by the spherical angles $(\theta, \phi)$, here denoted with $S_{\theta,\phi}$~\cite{arecchi1972atomic,agarwal1997atomic,ulyanov1999spin,lee2015visualizing}.
...
\end{equation}
where we introduced the notation $\ket{s,\theta,\phi}$ to denote SCSs with total spin number $s$ and $\mu=e^{i\phi}\tan(\theta/2)$.
...
\tmpHeading{Experimental engineering of SCSs}
% Although \acp{SCS} are in general not orthogonal, they form a convenient basis, analogously to coherent states.
As previously discussed, SCSs corresponding to opposite directions are orthogonal: $\braket{s,\theta,\phi}{s,\theta',\phi'}=0$ if $\bs n_{\theta,\phi}\cdot\bs n_{\theta',\phi'}=-1$.
...
% Furthermore, $\ket{S_1}$ and $\ket{S_2}$ are also eigenstates of the $S_x$ operator.
...
\tmpHeading{Cat states based on SCSs: phase-space picture}
We provide here a brief analysis of the features of SCSs in a phase space representation~\cite{agarwal1997atomic}. In particular, we shall be considering the analogous of the Husimi $Q$ function~\cite{walls2007quantum} defined as
\begin{equation}
...
\end{aligned}
\end{equation}
% in the spherical polar space where the Cartesian coordinates $(x,y,z)$ are mapped into $x\to Q_j(\alpha,\beta)\sin\alpha\cos\beta$, $y\to Q_j(\alpha,\beta)\sin\alpha\sin\beta$ and $z\to Q_j(\alpha,\beta)\cos\alpha$.
...
Using these expressions, we can compute $Q_\pm(\alpha,\beta)$ and investigate its features. However, looking at such function directly does not provide sufficient information for the discrimination of an incoherent mixture and a state such as $\ket{\psi_\pm}$.
...
\highlight{decide what put here}
