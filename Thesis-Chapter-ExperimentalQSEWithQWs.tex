...
\tmpHeading{Summary of the work} In this section we present an experimental implementation of the state engineering scheme discussed in~\cref{chapter:quantum_walks}, leveraging the \emph{coin space} of a \ac{QW} as an auxiliary degree of freedom to engineer target qudits.
...
The fidelity between experimentally synthesised and target states are computed by projecting the state on an orthonormal basis which contains the target state.
...
\end{tabular}
\caption{
	Amplitudes of the random target states. \highlight{come sono stati semplati?...}
